...
public class DeadzoneCamera : MonoBehaviour 
{
    ...
    public void Update()
    {
		if (target.GetComponent<PlayerPlatformerController> ().destoryCamera)
		{ 
        ...
        for (int i = 0; i < limits.Length; ++i)
        {
            if (limits[i].Contains(target.transform.position))
            {
				...
				smoothPos = Vector3.Max(smoothPos, localOffsetMin + new Vector3(0, Input.GetAxis ("Vertical")*2, 0));
				smoothPos = Vector3.Min(smoothPos, localOffsetMax + new Vector3(0, Input.GetAxis ("Vertical")*2, 0));
